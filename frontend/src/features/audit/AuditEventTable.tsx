            ...
            return (
              <Fragment key={event.id}>
                ...
                >
                  ...
                  {showObject && <Table.Td>{event.objectType === "API_KEY" ? `${event.objectDisplayName ?? event.objectId}…` : (event.objectDisplayName ?? event.objectId)}</Table.Td>}
                  <Table.Td>{event.objectType === "API_KEY" ? event.summary.replace(/(cr_[a-z]+_[a-f0-9]+)(?!…)/g, "$1…") : event.summary}</Table.Td>
                </Table.Tr>
