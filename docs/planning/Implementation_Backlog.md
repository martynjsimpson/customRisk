...
Tickets are grouped by implementation phase. Phase 0 is an implementation-only environment/bootstrap phase added before the six MVP product phases from MVP Scope. The MVP backlog ends at Phase 6. Post-MVP work must be tracked in `docs/planning/Post_MVP_Implementation_Backlog.md` using PM-prefixed ticket IDs. The backlog is intentionally implementation-oriented and must not introduce scope beyond the product and architecture documents.
...
Post-MVP improvements are deliberately excluded from this MVP backlog. Track them in `docs/planning/Post_MVP_Implementation_Backlog.md` using PM-prefixed ticket IDs.
