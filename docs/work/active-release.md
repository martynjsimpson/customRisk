...
**Fix:** Backend to add `reviewCommentMode` to the config snapshot (draft build + normalise + template publish write-back). Frontend to add draft-mode save handlers for both `reviewCommentMode` and `reviewAttestationText` that call `updateDraftConfig` when `draftConfigMode && hasDraft`, matching the pattern used by `responseActionMode`.
