...
**Fix:** Backend to add `reviewCommentMode` to `snapshotRegisterSettingsSchema`. Frontend to re-add draft-mode save handlers for both fields using `updateDraftConfig`, with onSuccess invalidating only `register-config` and `config-version-status` — NOT `["register", registerId]`.
