...
**Fix:** Updated `e2e:seed` script in root `package.json` to pass `--env-file=.env`; updated `e2e/fixtures/seed.ts` to use the driver adapter pattern matching the backend's Prisma instance. Additional schema mismatches discovered and corrected in a follow-up commit: invalid `value`/`minScore`/`maxScore`/`createdByUserId`/`updatedByUserId` on `LikelihoodValue`, `ImpactValue`, `RiskLevel`, `Risk`; missing `visibleToRoles`, `formulaDependencies`, `numericValue`, `responseStrategyId`. Seed now runs to completion. ✓
