...
**Fix:** Added `reviewCommentMode` and `reviewAttestationText` to `updateRegisterSchema` (validator) and to the Prisma `data` object in `updateRegister` (service). Audit trail entries added for both fields. Regression test added. 555 tests passing, typecheck clean. Fixed in commit 4c71132.
